...
def _clean(obj: Any, *, keep_full: bool, fields: list[str] | None) -> Any:
    if not isinstance(obj, dict):
        return obj
    # Projection first: an explicitly requested field always wins over the
    # default slimming, otherwise `--fields shownotes_html` would hand back a
    # null and an agent would read that as "this episode has no shownotes".
    if fields:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp.add_argument("--limit", type=int, default=20,
                    help="max episodes to return, always enforced (default 20)")
    sp.add_argument("--since", default=None,
                    help="inclusive YYYY-MM-DD lower bound; pages back to cover it")
    ...
    sp.add_argument(
        "--format",
        default="plain",
        choices=["plain", "timestamped", "segments"],
        help="plain/timestamped render text; segments returns the raw segment list",
    )
    sp.add_argument(
        "--with-segments",
        action="store_true",
        help="include the segment list alongside the rendered text",
    )
    sp.add_argument("--text", action="store_true",
                    help="print only the rendered text (no JSON wrapper)")
...
def run(args: argparse.Namespace) -> int | None:
    """Dispatch one subcommand. Returns an exit code, or None for success."""
